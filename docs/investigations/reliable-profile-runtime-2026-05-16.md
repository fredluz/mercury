...
- `ProfileRuntimeManager` and the runtime types in `src/main/hermes/types.ts` now centralize runtime identity, profile-keyed local gateway/API state, verified API/SSH-API runtime handles, pure remote fail-closed behavior, diagnostics, and stale-runtime markers. As of the 2026-05-21 API-only runtime change, local Hermes CLI fallback command identity is no longer current behavior.
...
- Chat, title generation, and cron API routing resolve a verified runtime handle before execution. API transports use the handle's URL/auth and reject mismatched or unverified profile identity; local chat/title no longer fall back to `hermes -p <profile>` command execution.
