...
Status update — 2026-05-31: this investigation's high-level toolset policy remains historical context, but endpoint references have moved with the Hermes gateway/runtime fixes. Current chat dispatch uses `/v1/runs` after verified `/health` + authenticated `/v1/capabilities`, not `/v1/chat/completions`, and runtime fixes intentionally refresh stale gateways rather than accepting compatibility fallback surfaces.
...
5. **Stale state blocks chat before any Hermes request.** `prepareChatBackend(...)` starts a local gateway only if needed and then calls `profileRuntimeManager.resolveRuntime(...)` (`src/main/services/chat-service.ts:125-170`). `resolveLocalApiRuntimeAttempt(...)` checks `state.staleReason` first and returns non-retryable `runtime-stale-after-profile-switch` before `/health`, `/v1/capabilities`, or `/v1/runs` is reached. This is why Mercury appears to require restart/revalidation after every toggle: Mercury blocks itself on the stale flag.
...
- Mercury readiness now uses `/health` plus authenticated `/v1/capabilities`. The capability descriptor advertises API features/endpoints and server-side tool execution, not currently loaded toolset names. So Mercury can verify desired config and runtime identity, but it cannot currently ask the running Hermes process “which toolsets are active?”
...
6. **Add regression tests.** Cover Tools toggle → config mirrors `cli`/`api_server`; no `runtime-stale-after-profile-switch` for hot-readable toolset toggles; rapid toggles batch any refresh; chat after toggle reaches `/v1/runs`; model/env/connection mutations still mark stale and restart/revalidate as today.
...
5. For SSH and remote runtimes, gate the hot-readable policy by evidence. If the remote Hermes version/capability is unknown, use the existing conservative refresh/revalidation path rather than assuming hot-read.
...
7. Add regression tests covering:
   ...
   - chat after a tool toggle reaches `/v1/runs` instead of failing with `runtime-stale-after-profile-switch`;
