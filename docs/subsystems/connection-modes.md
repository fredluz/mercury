...
- `probeHermesCapabilities(apiBaseUrl, authHeaders)` checks `<url>/health`, then performs an authenticated `GET /v1/capabilities`. The gate requires both `run_submission` and `session_resources`; Mercury deliberately does **not** treat runs-only compatibility as a sufficient target because normal chat/session UX depends on `/api/sessions` as well as `/v1/runs`. The probe also captures downstream run/session flags (`run_events_sse`, `run_stop`, `run_approval_response`) and classifies `401` `invalid_api_key` as a gateway API-key fault rather than a provider/model error.
...
- `ensureApiServerConfig(profile)` writes/refreshes Mercury's managed profile-specific API server config block in the selected profile's `config.yaml`. If older user-authored `api_server` text already exists, Mercury appends its managed block instead of preserving stale/disabled settings as authoritative; local port resolution reads the last `api_server` block so Mercury's enabled host/port wins. It is called during local initialization, not in remote modes.
- Local gateway startup runs Hermes via `HERMES_PYTHON` and `HERMES_SCRIPT`, with `HERMES_HOME`, enhanced `PATH`, and `HOME`. Before launch, Mercury reads the selected profile `.env`; if that profile has no `API_SERVER_KEY`, Mercury generates a `mercury_<uuid>` key and persists it back to the profile env. Profile env values are merged first, then Mercury forcibly injects `API_SERVER_ENABLED=true`, profile-specific `API_SERVER_HOST`/`API_SERVER_PORT`, and the persisted/generated `API_SERVER_KEY` into the child process environment. Applying the forced API server variables after profile `.env` merging prevents stale profile env from disabling or redirecting the Mercury-managed gateway API.
- Local runtime verification requires all managed-process evidence to match: Mercury-started child process, live process, expected host `127.0.0.1`, expected profile API port, expected gateway command, and a gateway PID file whose PID matches the child process. If everything except the PID file matches, Mercury reports a retryable "not ready yet" state while startup/revalidation continues. If only a profile `gateway.pid` exists with no live Mercury child in this process, `startGateway(profile)` treats that pid-file-only gateway as stale ownership, stops it, and starts a fresh Mercury-managed process instead of accepting the old API surface.
- Once local managed-process evidence is valid and `/health` succeeds, Mercury probes `/v1/capabilities` with the selected profile's persisted/generated `API_SERVER_KEY`. Missing `run_submission` or `session_resources` records an update-required diagnostic; invalid bearer auth records an invalid-auth diagnostic. Both states block chat/title execution instead of falling back to chat completions or runs-only compatibility.
...
- `startGateway(profile)` does nothing only when the current Mercury process still has a live child for that exact profile and returns `false` in that case. If only a profile `gateway.pid` process exists, Mercury treats it as pid-file-only/stale ownership, stops it, and starts a fresh managed gateway so the app does not stay attached to an old Hermes API surface.
- `stopGateway(force = false, profile?)` is profile-targeted. Omitting `profile` means the default profile, not all profiles. It only stops if the app started that profile gateway unless `force` is true; it also attempts to signal that profile's PID file and clears that PID file.
- `stopAllGateways()` is the app-shutdown path for local mode. It walks known runtime states plus `gateway.pid` files under the default home and `profiles/*`, signals each recorded gateway PID, and clears those PID files. Electron app lifecycle hooks call this explicit all-profiles function instead of relying on default-profile `stopGateway()` semantics.
