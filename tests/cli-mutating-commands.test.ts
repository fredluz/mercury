...
describe("mutating CLI commands", () => {
  ...
  it("mutates memory, user profile, SOUL, tools, skills, and gateway platform files", async () => {
    ...
    expect(
      (await runJson(home, ["memory", "add", "first memory"])).json.data,
    ).toEqual({ success: true });
    expect(
      (await runJson(home, ["memory", "update", "0", "updated memory"])).json
        .data,
    ).toEqual({ success: true });
    expect(readFileSync(join(home, "memories", "MEMORY.md"), "utf-8")).toBe(
      "updated memory",
    );

    expect(
      (await runJson(home, ["user-profile", "write", "--file", userFile])).json
        .data,
    ).toEqual({ success: true });
    expect(readFileSync(join(home, "memories", "USER.md"), "utf-8")).toBe(
      "User facts",
    );

    expect(
      (await runJson(home, ["soul", "write", "Soul text"])).json.data,
    ).toEqual({ success: true });
    expect(readFileSync(join(home, "SOUL.md"), "utf-8")).toBe("Soul text");
    expect((await runJson(home, ["soul", "reset"])).json.data.success).toBe(
      true,
    );

    expect(
      (await runJson(home, ["tools", "set", "web", "false"])).json.data,
    ).toEqual({ success: true, key: "web", enabled: false });

    const imported = await runJson(home, [
      "skills",
      "import",
      "--file",
      skillFile,
    ...
    expect(imported.json.data.skill).toMatchObject({
      name: "demo_skill",
      category: "custom",
    });
    expect(
      existsSync(join(home, "skills", "custom", "demo_skill", "SKILL.md")),
    ).toBe(true);

    const platform = await runJson(home, [
      "gateway",
      "platform",
      "set",
      "telegram",
    ...
    expect(platform.json.data).toEqual({
      success: true,
      platform: "telegram",
      enabled: true,
    });
    ...
    expect((await runJson(home, ["memory", "remove", "0"])).json.data).toEqual({
      success: true,
      index: 0,
    });
  });
  ...
  it("mutates env/config/model connection/model/credential state with JSON output", async () => {
    ...
    writeFileSync(
      entriesFile,
      JSON.stringify([{ key: "secret", label: "Primary" }]),
      "utf-8",
    );
    ...
    expect(
      (await runJson(home, ["env", "set", "OPENAI_API_KEY", "test-key"])).json
        .data,
    ).toMatchObject({ success: true, key: "OPENAI_API_KEY" });
    expect(readFileSync(join(home, ".env"), "utf-8")).toContain(
      "OPENAI_API_KEY=test-key",
    );

    expect(
      (await runJson(home, ["config", "set", "theme", "dark"])).json.data,
    ).toMatchObject({ success: true, key: "theme" });
    expect(readFileSync(join(home, "config.yaml"), "utf-8")).toContain(
      'theme: "dark"',
    );
    ...
    ).toMatchObject({ success: true, provider: "openai", model: "gpt-4.1" });
    expect(readFileSync(join(home, "config.yaml"), "utf-8")).toContain(
      'provider: "openai"',
    );
    ...
    ).toMatchObject({ success: true, mode: "remote" });
    expect(
      (
        await runJson(home, [
          "connection",
    ...
    ).toMatchObject({ success: true, host: "host", port: 2222 });

    const added = await runJson(home, [
      "models",
      "add",
      "--name",
      "Test",
    ...
    ).toMatchObject({ success: true, id });
    expect((await runJson(home, ["models", "remove", id])).json.data).toEqual({
      success: true,
      id,
    });
    ...
    ).toEqual({ success: true, provider: "openai", count: 1 });
    expect(
      JSON.parse(readFileSync(join(home, "auth.json"), "utf-8")).credential_pool
        .openai,
    ).toEqual([{ key: "secret", label: "Primary" }]);

    expect(
      (await runJson(home, ["sessions", "title", "set", "s1", "New", "Title"]))
        .json.data,
    ).toMatchObject({ success: true, sessionId: "s1", title: "New Title" });
    expect(
  ...
  it("round trips nested model config through the runtime config reader", async () => {
    const home = tempHome();
    writeFileSync(
      join(home, "config.yaml"),
    ...
    vi.resetModules();
    process.env.HERMES_HOME = home;
    const { getModelConfig } = await import("../src/main/config");
  ...
  it("supports mocked cron mutations and ndjson install progress without live services", async () => {
    ...
    vi.doMock("../src/main/services/cron-service", () => ({
      createCronJobForProfile: vi.fn(async () => ({
        success: true,
        id: "job-1",
      })),
      removeCronJobForProfile: vi.fn(async (id: string) => ({
        success: true,
        id,
      })),
      pauseCronJobForProfile: vi.fn(async (id: string) => ({
        success: true,
        id,
      })),
      resumeCronJobForProfile: vi.fn(async (id: string) => ({
        success: true,
        id,
      })),
      triggerCronJobForProfile: vi.fn(async (id: string) => ({
        success: true,
        id,
      })),
    }));
    ...
    ).toEqual({ success: true, id: "job-1" });
    expect((await runJson(home, ["cron", "pause", "job-1"])).json.data).toEqual(
      { success: true, id: "job-1" },
    );
    expect(
      (await runJson(home, ["cron", "resume", "job-1"])).json.data,
    ).toEqual({ success: true, id: "job-1" });
    expect((await runJson(home, ["cron", "run", "job-1"])).json.data).toEqual({
      success: true,
      id: "job-1",
    });
    expect(
      (await runJson(home, ["cron", "remove", "job-1"])).json.data,
    ).toEqual({ success: true, id: "job-1" });
